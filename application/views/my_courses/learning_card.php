 ...
 *
 * @var string   $mode             enrolled|invited|available|pending|rejected
 * @var object   $course
...
<article class="<?= $card_class ?> animate__animated animate__fadeInUp ec-stagger-<?= $i % 8 ?>"
  ...
  <?php elseif ($mode === 'pending'): ?>
    <span class="ec-card-status-badge ec-badge-inprogress">Pending</span>
  <?php elseif ($mode === 'rejected'): ?>
    <span class="ec-card-status-badge ec-badge-notstarted">Rejected</span>
    <?php endif; ?>
  </div>
  <div class="ec-card-body">
    ...
    <div class="ec-card-footer">
      ...
      <?php elseif ($mode === 'pending'): ?>
        <?php if ($cid > 0): ?>
        <a href="<?= base_url('courses/view/' . $cid . '?' . ka_lms_return_q('my_courses')) ?>" class="ec-card-cta ec-cta-continue">View request</a>
        <?php endif; ?>
      <?php elseif ($mode === 'rejected'): ?>
        <?php if ($cid > 0): ?>
        <a href="<?= base_url('index.php/courses/enroll/' . $cid) ?>" class="ec-card-cta ec-cta-start js-ka-enroll-retry">Request Again</a>
        <a href="<?= base_url('courses/view/' . $cid . '?' . ka_lms_return_q('my_courses')) ?>" class="ec-card-details-link">Details</a>
        <?php endif; ?>
      <?php else: ?>
        <?php if ($cid > 0): ?>
