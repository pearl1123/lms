...
// ── URI segments ──────────────────────────────────────────────
// segment(1) = controller  e.g. "dashboard", "my_courses", "courses"
// segment(2) = method      e.g. "create", "edit"
$segment = $this->uri->segment(1) ?? '';
$seg2    = $this->uri->segment(2) ?? '';

// ── Current user (stored as array in session by every controller) ──
$user      = $this->session->userdata('user') ?? [];
$user_role = strtolower($user['role']     ?? 'employee');
$full_name = $user['fullname']            ?? 'User';
$emp_id    = $user['employee_id']         ?? '';

// Avatar initials (max 2 letters)
$initials = '';
foreach (explode(' ', trim($full_name)) as $word) {
...
<style>
  ...
  .ka-sidebar-scroll::-webkit-scrollbar {
    width: 3px;
  }

  .ka-sidebar-scroll::-webkit-scrollbar-thumb {
  ...
  .ka-nav-link.active .ka-nav-icon {
    opacity: 1;
  }
  ...
  .ka-nav-submenu .ka-nav-link:hover {
    color: #fff;
    background: rgba(255, 255, 255, 0.06);
  }

  .ka-nav-submenu .ka-nav-link.active {
    color: var(--ka-primary, #6dabcf);
    background: rgba(109, 171, 207, 0.15);
  }

  .ka-nav-submenu .ka-nav-link.active::before {
    display: none;
  }
  ...
  .ka-nav-link[aria-expanded="true"] .ka-nav-caret {
...
<aside class="ka-sidebar" id="kaSidebar">
  ...
  <!-- Brand -->
  <a href="<?= base_url('dashboard') ?>" class="ka-sidebar-brand">
  ...
  <nav class="ka-sidebar-scroll">
    ...
    <ul class="ka-nav-list">
      ...
      <li class="ka-nav-item">
        <a href="<?= base_url('my_courses') ?>"
          class="ka-nav-link <?= $segment === 'my_courses' ? 'active' : '' ?>">
          <svg class="ka-nav-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z" />
            <path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z" />
          </svg>
          <?= $user_role === 'employee' ? 'My Learning' : 'My Courses' ?>
        </a>
      </li>
        ...
        <?php if ($user_role === 'admin'): ?>
          <li class="ka-nav-item">
            ...
            <ul class="ka-nav-submenu collapse <?= in_array($segment, ['manage_courses', 'categories']) ? 'show' : '' ?>"
              ...
              <li>
                <a href="<?= base_url('manage_courses/create') ?>"
    ...
    <ul class="ka-nav-list">
      ...
      <li class="ka-nav-item">
        <a href="#manageadministratorMenu"
          class="ka-nav-link <?= in_array($segment, ['manage_courses', 'categories']) ? 'active' : '' ?>"
          data-bs-toggle="collapse"
          aria-expanded="<?= in_array($segment, ['manage_courses', 'categories']) ? 'true' : 'false' ?>">
          <svg class="ka-nav-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <ul class="ka-nav-submenu collapse <?= in_array($segment, ['manage_courses', 'categories']) ? 'show' : '' ?>"
          ...
          <li>
            <a href="#manageadministratorMenuPermLibraries"
              class="ka-nav-link "
              data-bs-toggle="collapse"
              aria-expanded="<?= in_array($segment, ['manage_courses', 'categories']) ? 'true' : 'false' ?>">
              Permissions Library
            ...
            <ul class="ka-nav-submenu collapse "
              id="manageadministratorMenuPermLibraries">
              <li>
                <a href="<?= base_url('manage_courses') ?>"
                  class="ka-nav-link <?= ($segment === 'manage_courses' && ! $seg2) ? 'active' : '' ?>">
                  Permissions List
  ...
  <div class="dropdown" style="flex-shrink:0;">
    ...
    <ul class="dropdown-menu dropdown-menu-end mb-1" style="min-width:200px;">
      ...
      <li><a class="dropdown-item" href="<?= base_url('profile') ?>">
          <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-sm me-2 text-muted" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
            <circle cx="12" cy="7" r="4" />
          </svg>
          My Profile
      ...
      <li><a class="dropdown-item text-danger" href="<?= base_url('auth/logout') ?>">
          <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-sm me-2" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
