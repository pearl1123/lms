    ...
    <div class="modal-content border-0 rounded-4 overflow-hidden shadow-lg animate__animated animate__fadeInDown animate__faster" style="backdrop-filter: blur(5px);">
      ...
      <form id="registerForm" action="<?= base_url('auth/register_process'); ?>" method="post" autocomplete="off" novalidate class="p-3">
        <div class="row g-3">
          ...
          <div class="col-12 position-relative animate__animated animate__bounceIn animate__faster animate__delay-0-1s">
            ...
            <div class="input-group">
              <input type="text" name="employee_id" id="employee_id" class="form-control form-control-lg rounded-start-pill shadow-sm" placeholder="Enter Employee ID" autocomplete="one-time-code" readonly
                onfocus="this.removeAttribute('readonly');" required>

              <button type="button" id="checkEmployeeBtn" class="btn btn-primary px-4 rounded-end-pill shadow-sm" style="background: linear-gradient(90deg,#6dabcf,#5a9ec1);">Verify ID</button>
            </div>
          </div>

          <span id="employee_icon" class="position-absolute end-0 top-50 translate-middle-y me-3" style="font-size:1.2rem; display:none; z-index: 5;"></span>
          <div id="employee_tooltip" class="mt-1 small">Enter your employee ID exactly as provided by HR.</div>
        ...
        <div class="col-12 animate__animated animate__bounceIn animate__faster animate__delay-0-2s">
          ...
          <input type="text" name="emp_name" id="emp_name" class="form-control form-control-lg rounded-pill shadow-sm input-lift" placeholder="Employee Name" required readonly>
        </div>

        <div class="col-12 animate__animated animate__bounceIn animate__faster animate__delay-0-2s">
          <label class="form-label fw-semibold">Department</label>
          <input type="text" name="department" id="department" class="form-control form-control-lg rounded-pill shadow-sm input-lift" placeholder="Employee Department" required readonly>
        </div>
...
<script>
  $(document).ready(function() {
    ...
    $('.terms-toc a').on('click', function(e) {
      ...
      $('#termsContent').animate({
        scrollTop: $(target).position().top + $('#termsContent').scrollTop()
      }, 400);
    });
  });
      ...
      success: function(response) {
        if (response.success) {
          ...
          if(response.registered) {
            ...
            $('#emp_name, #department, #password, #confirm_password, #agree_terms, #register_submit').prop('disabled', false);
          }
          // $('#employee_icon').html('✓').css('color', 'green').show();
        } else {
          
          // $('#employee_status').text(response.message).css('color', 'red');
          // $('#employee_icon').html('✗').css('color', 'red').show();
        }
      },
      error: function() {
